...
def download_bible(version):
    ...
    url = f"https://github.com/{repo_owner}/{repo_name}/releases/download/latest/{version}.mybible"
...
def list_available_online_versions():
    ...
    url = f"https://github.com/{repo_owner}/{repo_name}/releases/download/latest/index.json"
...
def main():
    ...
    parser.add_argument('-d', '--download', help='Download a Bible version')
    ...
    # If we're not listing versions or books, then verse is required
    if not args.verse:
